...
module "dcos-core" {
  ...
  bootstrap_os_user    = module.dcos-infrastructure.bootstrap_admin_username
  ...
  masters_os_user    = module.dcos-infrastructure.masters_admin_username
  num_masters        = var.num_masters
  ...
  dcos_fault_domain_detect_contents = coalesce(
    var.dcos_fault_domain_detect_contents,
    file("${path.module}/scripts/fault-domain-detect.sh"),
  )
  ...
  dcos_ip_detect_contents = coalesce(
    var.dcos_ip_detect_contents,
    file("${path.module}/scripts/ip-detect.sh"),
  )
  dcos_ip_detect_public_contents = coalesce(
    var.dcos_ip_detect_public_contents,
    file("${path.module}/scripts/ip-detect-public.sh"),
  )
